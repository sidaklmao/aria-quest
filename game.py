...
victory_bg = pygame.image.load(resource_path("greenery_bg.png"))
...
while running:
    if game_started:
        ...
        if can_shoot:
            # Shadow for "Shoot Now!"
            shadow_x, shadow_y = WIDTH // 2 - 52, HEIGHT - 82
            main_x, main_y = WIDTH // 2 - 50, HEIGHT - 80
            draw_text("Shoot Now!", shadow_x, shadow_y, BLACK )  # Shadow in red
            draw_text("Shoot Now!", main_x, main_y, RED)   # Main text in black                

        if not shield_ready:
            # Shadow for "Shield Cooldown"
            shadow_x, shadow_y = WIDTH // 2 - 102, HEIGHT - 122
            main_x, main_y = WIDTH // 2 - 100, HEIGHT - 120
            draw_text(f"Shield Cooldown: {shield_cooldown // 60 - shield_timer // 60}s", shadow_x, shadow_y, BLACK)  # Shadow
            draw_text(f"Shield Cooldown: {shield_cooldown // 60 - shield_timer // 60}s", main_x, main_y, RED)   # Main text
        elif not shield_active:
            # Shadow for "Shield Ready!"
            shadow_x, shadow_y = WIDTH // 2 - 102, HEIGHT - 122
            main_x, main_y = WIDTH // 2 - 100, HEIGHT - 120
            draw_text("Shield Ready! (Press D)", shadow_x, shadow_y, BLACK)  # Shadow in red
            draw_text("Shield Ready! (Press D)", main_x, main_y, RED)   # Main text in black

        if game_over:
            draw_text("GAME OVER", WIDTH // 2 - 100, HEIGHT // 2, RED)
            draw_text("Press R to Restart", WIDTH // 2 - 120, HEIGHT // 2 + 40, RED)
            if keys[pygame.K_r]:
                current_boss_level = 0
                reset_level()
            ...
            if keys[pygame.K_r]:
                ...
                reset_level()
        if victory:
            # Draw greenery background
            screen.blit(victory_bg, (0, 0))  # Assuming victory_bg is the greenery background

            # Display the "restored Earth" message
            victory_text = "Thank you Aria. You have restored Earth to its former vitality."
            text_surface = story_font.render(victory_text, True, WHITE)
            screen.blit(
                text_surface,
